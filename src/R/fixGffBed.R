...
names(bed6) <- paste0(gsub("\\.bed.*", "", sapply(bed9files, basename)), '.bed6')

bed6[["irgsp1_rRNA_tRNA.bed6"]]$V1 <- gsub("0(\\d)", "\\1",
                                         gsub("chr", "Chr",
                                              bed6[["irgsp1_rRNA_tRNA.bed6"]]$V1,
                                              fixed = TRUE))
  
for (i in 1:length(bed6)) {
  outpath <- paste0(outdir, "/", names(bed6)[i])
  write.table(bed6[i], file = outpath, quote = FALSE, sep = "\t", row.names = FALSE, col.names = FALSE)
}
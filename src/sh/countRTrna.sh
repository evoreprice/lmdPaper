...
for bam in $bamfiles; do
	...
	srun --ntasks=1 --cpus-per-task=1 --exclusive --output="$outdir"/"$library".rrna.tmp $cmdR &
	cmdT="samtools view -c -F0x100 -L "$outdir"/rap_tRNA.bed9 "$bam""
	srun --ntasks=1 --cpus-per-task=1 --exclusive --output="$outdir"/"$library".trna.tmp $cmdT &
done

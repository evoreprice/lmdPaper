...
Examination of the clustered genes suggested that transcription factors were overrepresented in most clusters (**annotated cluster tables in SI**). To determine whether particular TF families were associated with particular patterns of expression, the enrichment of TF family members in each cluster was calculated using hypergeometric tests. Using the list of transcription factor families available on the Plant Transcription Factor Database (**reference**), a hypergeometric test was carried out if there were at least two members of TF family in a given cluster. After adjustment for multiple comparisons, each cluster had a significant enrichment of at least one TF family (`r I(pasteLabel("Table ", tableCount, "t_hypergeom"))`). For example, members of the ABI3VP1 family were overrepresented in clusters **2** and **4**, both of which contain genes with higher transformed read counts in the rachis meristem than in samples from later stages of development. The MADS family was overrepresented in clusters **6** and **8**, suggesting that they could be involved in both early and late stages of inflorescence development.

* Clusters including characterised genes

## Dynamic expression of transcription factor families

Because of the enrichment of TF family members in some of the gene expression clusters, geneset enrichment analysis (GSEA) was used to provide an overview of TF family expression in the meristem samples (`r I(pasteLabel("Figure ", figCount, "f_gsea"))`). In general, the GSEA results correspond to the hypergeometric tests for transcription factor enrichment in the clusters. For example, SBP transcription factors are enriched in RM and PBM, consistent with their overrepresentation in cluster **6**, which contains genes with higher expression in those meristem types.

The patterns are not completely representative of all members of each gene family. For example, there are several MADS genes that have lower expression in the SM than in the RM, but expressed MADS genes were detected at a higher level overall in SM. Nevertheless, there are some notable modes of expression shared between different TF families. Several TF families, including bZIP and ABI3VP1, are strongly expressed in RM and lower in the other meristem types. Another group incluiding Squamosa binding proteins and Jumonji transcription factors is more weakly expressed in the RM but enriched in the PBM, suggesting a switch of expression between these two meristem types. Correspondingly, the most pronounced difference in the GSEA was that between the RM and PBM samples.

TF families involved in branching were enriched in **which families, which samples**. 
...
## Comparison with characterised genes

To compare previously reported expression patterns with the results presented here, which were produced with high-throughput sequencing, a selection of studies of corresponding stages of inflorescence development were searched for qualitative *in situ* hyrbridisation data.

```{r f_reviewInSitu, fig.height = 8.661 / 4}
print(f_reviewInSitu)
```

* **Table**: our expression calls (yes/no by stage) *vs* in situ reports. Fun times.
* our in situ results could go here

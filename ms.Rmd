...
The presence and co-expression of genes that are involved in inflorescence development along with other uncharacterized TF genes indicates that the clusters capture biologically meaningful groups of genes. The G1-Like (G1L/ALOG) genes were retrieved by querying Phytozome 10.3 for the Pfam domain PF04852 [@Goodstein:2012ho; @Finn:2014kj]. Only three were detected in the dataset. Two of them, *G1L2* and *G1L5* (*TAWAWA1*), were present in clusters `r getClusterName('dec', 'small')` and `r getClusterName('dec', 'dec')` respectively, and the third, *G1L1*, was not clustered but appears to have a similar expression pattern (`r I(pasteLabel("Figure ", figCount, "f_alogFamily"))`a). Overexpression of *G1L5* increases the production of secondary branches and leads to the formation of tertiary branches, potentially by suppressing the transition from BM to SM identity, resulting in higher grain yield [@Yoshida:2013ff]. *G1L5* missense mutants have small inflorescences with reduced grain yield, but this phenotype was stronger when *G1L5* was targeted by RNAi [@Yoshida:2013ff]. This might be explained by redundancy between the three co-expressed G1L genes, *G1L1*, *G1L2* and *G1L5*, with all three of these genes being affected by the RNAi construct used against *G1L5*. Yeast two-hybid experiments indicate that G1L5 activates transcription of two *STMADS11*-like (*SVP*-like) MADS genes, *MADS22* and *MADS55*, but activation of the third rice *STMADS11*-like gene, *MADS47*, is not reported [@Yoshida:2013ff]. *MADS22* and *MADS55* are also enriched in the RM, and *MADS47* is present in cluster `r getClusterName('dec', 'small')` with *G1L2*, although it is not highly expressed (`r I(pasteLabel("Figure ", figCount, "f_alogFamily"))`b).
...
| **Shorten MADS section and insert here. MADS DEG figure goes to SI?**
| **GRAS TFs (SHR1/SCR)?**

The strong expression of SQUAMOSA promoter binding protein-like (SPL/SBP) genes in the RM and PBM and their presence in clusters `r getClusterName('small', 'dec')` and `r getClusterName('dec', 'small')` suggests involvement in the early stages of inflorescence development. *WEALTHY FARMER'S PANICLE* (*WFP*/*SPL14* or *IDEAL PLANT ARCHITECTURE 1*/*IPA1*), *SPL3*, *SPL12* and *SPL17* are all highly expressed in the RM, gradually decrease in expression and have weakest expression in the SM. These genes are all putative targets of *miR156*, a microRNA that causes reduced panicle size and delayed flowering when overexpressed [@Xie:2006dc]. Overexpression of *WFP*, which was detected by *in situ* hybridisation experiments in the RM and branch meristems in wild-type plants, is associated with increased branching and grain yield [@Jiao:2010ft; @Miura:2010it]. WFP binds to the promoter region and appears to cause upregulation of the *DENSE AND ERECT PANICLE 1* (*DEP1*) gene [**ref Lu 2013**], which is present in cluster `r getClusterName('inc', 'dec')` and encodes a major QTL for grain production and panicle morphology in high-yield *O. sativa* ssp. *japonica* varieties. Probable loss-of-function *dep1* mutants have larger SAMs with higher cell number, and reduced expression of *GN1A* [@Huang:2009kf]. WFP also binds to the promoters of *LOG1*, *PIN PROTEIN 1B* and *SLENDER RICE1*, suggesting a role in the coordination of hormone signalling [**ref Lu 2013**].

| **[Re]move following MADS section and insert section on Homeobox. Can link via WUS (HB, target of cytokinin, e.g. Han mini-review) and/or OSH1 (HB, downregulated in LOG mutant, e.g. Kurakawa 2007)**
...
```{r f_alogFamily, fig.width = 3.150, fig.height = 3.150 * 3/2}
grid::grid.draw(f_alogFamily)
```

**`r I(pasteLabel("Figure ", figCount, "f_alogFamily"))`**. Expression of detected G1-Like (G1L/ALOG)  (**a**) and *STMADS11*-like MADS (**b**) genes in inflorescence meristems. Red or blue points indicate whether the gene was detected or not detected respectively for each library. Only three G1L genes (*G1L1*, *G1L2* and *G1L5*/*TAWAWA1*) were detected, but they share a similar pattern of expression. Two reported *STMADS11*-like (*SVP*-like) targets of G1L5, *MADS22* and *MADS55* [@Yoshida:2013ff], are also more strongly expressed in the RM than in other meristems, but the other *STMADS11*-like gene, *MADS47*, was only detected in one library.
